...
from peft_pretraining import custom_activations
...
class GPASBlock(nn.Module):
    def __init__(self, gate_init_value = 0.0, act_fn = 'silu', requires_grad = True):
        super().__init__()
        self.gate = nn.Parameter(torch.tensor(gate_init_value), requires_grad=requires_grad)
        self.act_fn = custom_activations.ACT2FN[act_fn]
    
    def forward(self, hidden_states):
        return hidden_states - self.act_fn(self.gate) * hidden_states.detach()
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_index: int):
        ...
        try:
            self.max_post_norm_layer = int(os.getenv('POST_NUM', '0'))
        except ValueError as e:
            print("Setting max_post_norm_layer = 0")
            self.max_post_norm_layer = 0
        ...
        if norm_type == 'pre' or norm_type == 'scale_pre':
            ...
            if norm_type == 'deeppost':
                self.alpha = ((2 * self.layer_nums)**0.25)
            ...
            if self.layer_index < self.max_post_norm_layer:
                ...
                self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
                self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        elif norm_type in ['pre_gpas', 'lns_gpas']:
            self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.gpas_block = GPASBlock()
            if norm_type == 'lns_gpas':
                self.input_layernorm.weight.data.fill_(1/math.sqrt(layer_index+1))
                self.post_attention_layernorm.weight.data.fill_(1/math.sqrt(layer_index+1))
        elif norm_type == 'sandwich_gpas':
            self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.pre_feedforward_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_feedforward_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.gpas_block = GPASBlock()
        elif norm_type == 'deeppost_gpas':
            self.post_feedforward_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            self.alpha = ((2 * self.layer_nums)**0.25)
            self.gpas_block = GPASBlock()
        elif norm_type == 'post_pre_gpas':
            print("cur layer index is:", layer_index)
            self.gpas_block = GPASBlock()
            if self.layer_index < self.max_post_norm_layer:
                print(f"cur layer is {layer_index}, and you're using the post norm!")
    ...
    def forward(
        ...
        if norm_type == 'pre' or norm_type == 'scale_pre':
            ...
            hidden_states = residual + hidden_states
            hidden_states = self.gpas_block(hidden_states)
            ...
            # Post-LayerNorm Only
            residual = self.gpas_block(hidden_states)
            hidden_states, self_attn_weights, present_key_value = self.self_attn(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            # We use scaled initialization for token embeddings to stabilize training,
            # which was proposed in "Spike No More": https://arxiv.org/abs/2312.16903v2
            module.weight.data.normal_(mean=0.0, std=(2 / 5) ** 0.5)

            # You can also try the default initialization below,
            # which we've tested on 350M Pre-LN and Pre + GPAS.
            # PPL: 20.49 vs. 19.37 (1.12 PPL improvement from GPAS).
            # module.weight.data.normal_(mean=0.0, std=std) # default init
